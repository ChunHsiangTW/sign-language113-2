...
class SignLanguageDataset(Dataset):
    ...
    回傳：x:[T,330]（不在這裡做 padding）、y:int、length:int(=T)
    """
    ...
    def __getitem__(self, idx):
        ...
        return x, y, T  # 可變長度：把實際長度帶出去

def collate_fn(batch):
    """
    batch: list of (x:[T_i,330], y:int, len_i:int)
    回傳：
      xs     : [B, T_max, 330]（用 0 在時間維 padding）
      ys     : [B]
      lengths: [B]（原始 T_i）
    """
    xs_list, ys_list, lens_list = zip(*batch)  # tuple of tensors/ints
    lengths = torch.tensor(lens_list, dtype=torch.long)
    # pad_sequence 會把較短的自動補到該批最長長度
